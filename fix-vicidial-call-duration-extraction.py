#!/usr/bin/env python3
"""
Fix ViciDial Call Duration Extraction - Enhanced reliability for call time extraction
"""
...
def enhanced_call_duration_extraction(page_html, lead_id=None):
    """
    Enhanced call duration extraction with improved reliability
    Uses multiple strategies and better pattern matching
    """
    call_info = {
        'call_duration': None,
        'call_timestamp': None,
    ...
    try:
        ...
        recordings_text = None

        # Find text containing "RECORDINGS FOR THIS LEAD"
        recordings_markers = soup.find_all(text=re.compile(r"RECORDINGS FOR THIS LEAD", re.IGNORECASE))
                                ...
                                if seconds_col_index is not None and seconds_col_index < len(data_cells):
                                    ...
                                    if seconds_match:
                                        ...
                                        if 1 <= seconds <= 3600:  # Reasonable bounds
                                            call_info['call_duration'] = format_duration(seconds)
                                            call_info['talk_time'] = format_talk_time(seconds)
                ...
                if os.path.exists(recording_path):
                    try:
                        ...
                        if result.returncode == 0 and result.stdout.strip():
                            ...
                            call_info['call_duration'] = format_duration(duration_seconds)
                            call_info['talk_time'] = format_talk_time(duration_seconds)
...
def test_with_sample_html():
    """Test function with sample ViciDial HTML"""

    # Sample HTML that might be found in ViciDial
    sample_html = """
    <html>
    <body>
    ...
    result = enhanced_call_duration_extraction(sample_html, "132511")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        print("Enhanced ViciDial call duration extraction utility loaded")
        print("Usage: python3 fix-vicidial-call-duration-extraction.py test")